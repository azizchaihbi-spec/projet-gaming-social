...
<body class="relative min-h-screen overflow-x-hidden smooth-scroll">
  ...
  <main class="container mx-auto px-6 py-12 max-w-7xl">
    ...
    <div class="card rounded-3xl p-10 glow mb-10" id="filterSection">
      ...
      <form method="GET" action="<?= basename($_SERVER['PHP_SELF']) ?>" id="filterForm" class="flex flex-col md:flex-row gap-4">
  ...
  <script>
    ...
    function filterByForum(forumId) {
      window.location.href = '<?= basename($_SERVER['PHP_SELF']) ?>?id_forum=' + forumId;
    }
    ...
    function saveEdit(id) {
      ...
      fetch('../../api_admin.php?action=edit_post', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
